    ...
    void Activate() const {
        ...
        for (const auto &item: this->uniformMap) {
            if(std::holds_alternative<float>(item.second)) {
                const auto value = std::get<float>(item.second);
                const auto uni = glGetUniformLocation(this->shader->ID, item.first.c_str());
                glUniform1f(uni, value);
            }
            if(std::holds_alternative<glm::vec3>(item.second)) {
                const auto value = std::get<glm::vec3>(item.second);
                const auto uni = glGetUniformLocation(this->shader->ID, item.first.c_str());
                glUniform3fv(uni, 1, &value[0]);
            }
            if(std::holds_alternative<glm::mat4>(item.second)) {
                const auto value = std::get<glm::mat4>(item.second);
                const auto uni = glGetUniformLocation(this->shader->ID, item.first.c_str());
                glUniformMatrix4fv(uni, 1, GL_FALSE, &value[0][0]);
            }
            if(std::holds_alternative<Texture*>(item.second)) {
                const auto value = std::get<Texture*>(item.second);
                const auto texUni = glGetUniformLocation(this->shader->ID, item.first.c_str());
                glUniform1i(texUni, static_cast<GLint>(value->unit));
                value->Bind();
            }
        }
